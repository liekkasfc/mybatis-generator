...
public class MybatisServicePlugin2 extends PluginAdapter {
	...
	public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
		...
		pagerType=new FullyQualifiedJavaType("com.yolipai.server.common.utils.Pager");
	...
	private Method getPagerEntitys(IntrospectedTable introspectedTable,
		...
		method.addBodyLine("int start = Integer.valueOf(params.getOrDefault(\"start\", 0).toString());");
		method.addBodyLine("int size  = Integer.valueOf(params.getOrDefault(\"size\", 10).toString());");
		...
		method.addBodyLine("criteria.setLimitStart(start);");
		method.addBodyLine("criteria.setLimitEnd(size);");
		...
		List<IntrospectedColumn> introspectedColumnsList=introspectedTable.getAllColumns();
		 for (IntrospectedColumn introspectedColumn : introspectedColumnsList) {
			 ...
			 if(introspectedColumn.isStringColumn()){
				 ...
				 method.addBodyLine("\tcriteria.and"+toUpperCase(javaProperty)+"EqualTo(params.getOrDefault(\""+javaProperty+"\", \"\").toString());");
			 }
